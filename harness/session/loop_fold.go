...
func (f *loopFold) rememberQueue(id string, kind QueueKind, blocks []sessionloop.InputBlock, position sessionloop.Position) {
	f.queue[id] = loopQueueFact{kind: loopCommandKind(kind), blocks: blocks, position: position}
}
...
func (f *loopFold) queuedInput(id string) sessionloop.QueuedInput {
	...
	return sessionloop.QueuedInput{
		...
		Blocks:   cloneLoopInputBlocks(fact.blocks),
	}
}
...
func cloneLoopInputBlocks(blocks []sessionloop.InputBlock) []sessionloop.InputBlock {
	if blocks == nil {
		return nil
	}
	clones := make([]sessionloop.InputBlock, len(blocks))
