...
// TestLiveSuspensionProjectionWaitsForRunFinalization freezes the ordering
// required by the protocol view: the driver's durable run.suspended record is
// not observable as StateSuspended until the owning drive has finalized the
// inner session. Pure replay has no hook and remains immediate.
func TestLiveSuspensionProjectionWaitsForRunFinalization(t *testing.T) {
	payloadCodec := jsoncodec.New()
	payload, err := codec.Encode(payloadCodec, event.SuspensionPayload{
		Suspension: agentic.Suspension{ID: "susp-1", Kind: "test"},
	})
	if err != nil {
		t.Fatal(err)
	}
	record := event.Record{
		Cursor: 7, Nature: agentic.EventLifecycle, Source: "agentic",
		Type: agentic.EventTypeRunSuspended, Payload: payload,
	}
	projector := newLoopProjector("session-under-test", payloadCodec, nil)
	projector.fold.currentRunID = "run-1"
	entered := make(chan struct{})
	release := make(chan struct{})
	projector.awaitRunFinalized = func(ctx context.Context, runID string) error {
		if runID != "run-1" {
			return fmt.Errorf("awaited run %q, want run-1", runID)
		}
		close(entered)
		select {
		case <-release:
	...
	go func() {
		...
		result <- projectionResult{events: events, err: applyErr}
	}()
	awaitSignal(t, entered, "live suspension finalization wait")
	select {
	case early := <-result:
		t.Fatalf("run.suspended projected before finalization: events=%#v err=%v", early.events, early.err)
	default:
	}
	close(release)
	projected := <-result
	...
	if len(projected.events) != 1 || projected.events[0].Kind != sessionloop.EventRunSuspended ||
		projected.events[0].State != sessionloop.StateSuspended {
		t.Fatalf("finalized suspension projection = %#v", projected.events)
	}

	replay := newLoopProjector("session-under-test", payloadCodec, nil)
	replay.fold.currentRunID = "run-1"
	replayed, err := replay.apply(context.Background(), record)
	if err != nil || len(replayed) != 1 || replayed[0].Kind != sessionloop.EventRunSuspended {
		t.Fatalf("replay suspension projection = %#v, %v", replayed, err)
	}
}
...
func TestLoopProjectionCoversEveryJournalKind(t *testing.T) {
	...
	if assistantEntry == nil || len(assistantEntry.Blocks) != 2 ||
		assistantEntry.Blocks[0].Kind != sessionloop.EntryBlockText ||
		assistantEntry.Blocks[1].Kind != sessionloop.EntryBlockToolCall ||
		string(assistantEntry.Blocks[1].ToolCall.Data) != `{"q":"x"}` {
		t.Fatalf("assistant entry = %#v", assistantEntry)
	}
	if jsonTool == nil || len(jsonTool.Blocks) != 1 ||
		string(jsonTool.Blocks[0].Data) != `{"found":true}` ||
		jsonTool.Blocks[0].ToolResult.CallID != "call-1" || jsonTool.Blocks[0].ToolResult.IsError {
		t.Fatalf("json tool entry = %#v", jsonTool)
	}
	if plainTool == nil || plainTool.Blocks[0].Data != nil ||
		plainTool.Blocks[0].Text != "plain text result" || !plainTool.Blocks[0].ToolResult.IsError {
