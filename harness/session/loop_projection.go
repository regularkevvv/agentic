...
//	queue.accepted              EventQueueAccepted (QueuedInput{ID, Kind, Position, Blocks}; the fold records ID->kind for later attribution).
...
// incremental fold. The attribution, run-finalization, and output hooks are
// installed by the live LoopView; a pure replay leaves them nil and yields
// empty command IDs and no structured output — replayed history cannot invent
// live-host attribution or wait on process-local execution.
...
func (p *loopProjector) applyAgentic(ctx context.Context, record event.Record) ([]sessionloop.Event, error) {
	switch record.Type {
	...
	case agentic.EventTypeToolResultCommitted:
		...
		block := sessionloop.EntryBlock{
			Kind: sessionloop.EntryBlockToolResult,
			Text: payload.Content,
			ToolResult: &sessionloop.EntryToolResult{
	...
	case agentic.EventTypeRunSuspended:
		...
		// The driver emits and durably records run.suspended synchronously just
		// before it returns. The inner session becomes Suspended only when its
		// drive-finalization path consumes that return. Do not expose a live
		// protocol event whose State says suspended while Snapshot or Close can
		// still observe Running. Replay has no process-local hook and remains
		// immediate because the durable history is already finalized.
		if p.awaitRunFinalized != nil {
			if err := p.awaitRunFinalized(ctx, p.fold.currentRunID); err != nil {
				return nil, err
			}
		}
...
// projectMessageToEntryBlocks is the explicit egress authority boundary from
// Agentic's complete model-facing Message representation to sessionloop's
// complete, durable, consumer-facing EntryBlock representation. Text parts
...
func projectMessageToEntryBlocks(message agentic.Message) []sessionloop.EntryBlock {
	...
	for _, part := range message.Content {
		...
		case agentic.ContentText:
			if part.Text != "" {
				blocks = append(blocks, sessionloop.EntryBlock{Kind: sessionloop.EntryBlockText, Text: part.Text})
			}
		case agentic.ContentToolUse:
			...
			block := sessionloop.EntryBlock{
				...
				ToolCall: &sessionloop.EntryToolCall{
...
// projectMessageToInputBlocks reconstructs the provider-neutral caller input
// represented by a durable Harness queue fact. Agentic's current queue accepts
// user Messages, while this v1 host supports only text protocol input; provider
// and execution-only parts are deliberately not reclassified as caller input.
func projectMessageToInputBlocks(message agentic.Message) []sessionloop.InputBlock {
	var blocks []sessionloop.InputBlock
	for _, part := range message.Content {
		if part.Type == agentic.ContentText && part.Text != "" {
			blocks = append(blocks, sessionloop.InputBlock{Kind: sessionloop.InputBlockText, Text: part.Text})
		}
	}
	return blocks
}
