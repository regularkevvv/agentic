...
func TestLoopResumeRequestConversion(t *testing.T) {
	...
	badInput.Input = &sessionloop.Input{Content: []sessionloop.Block{{Kind: sessionloop.BlockData, Data: json.RawMessage(`1`)}}}
	if _, err := loopResumeRequest(badInput); !errors.Is(err, sessionloop.ErrUnsupported) {
		t.Fatalf("unsupported continuation err = %v", err)
	}
}

func TestLoopBlockAndRoleHelpers(t *testing.T) {
	...
	if cloneLoopBlocks(nil) != nil {
		t.Fatal("cloneLoopBlocks(nil) != nil")
	}
	cloned := cloneLoopBlocks(blocks)
	cloned[0].Text = "mutated"
	if blocks[0].Text != "kept" {
		t.Fatal("cloneLoopBlocks shares memory")
	}
}
...
func TestLoopViewDispatchBranchCoverage(t *testing.T) {
	...
	if _, err := plain.Dispatch(loopTestContext(t), sessionloop.Command{
		...
		Input: &sessionloop.Input{Content: []sessionloop.Block{
			{Kind: sessionloop.BlockData, Data: json.RawMessage(`1`)},
		}},
