...
func TestLoopViewStartLifecycle(t *testing.T) {
	...
	if len(entries) != 1 || entries[0].Origin != sessionloop.OriginStart ||
		...
		entries[0].RunID != receipt.RunID || len(entries[0].Content) != 1 ||
		entries[0].Content[0].Text != "hello loop" {
...
func TestLoopViewProjectionReplayEquality(t *testing.T) {
	...
	for _, entry := range streamed {
		...
		if len(entry.Content) > 0 && entry.Content[0].Kind == sessionloop.BlockText {
			texts = append(texts, entry.Content[0].Text)
		}
	}
	...
	var toolCall *sessionloop.ToolCall
	var toolResult *sessionloop.ToolResult
	for _, entry := range streamed {
		for _, block := range entry.Content {
			if block.Kind == sessionloop.BlockToolCall {
				toolCall = block.ToolCall
			}
			if block.Kind == sessionloop.BlockToolResult {
...
func TestLoopViewDispatchValidationAndUnsupportedInput(t *testing.T) {
	...
	dataStart := sessionloop.Command{Kind: sessionloop.CommandStart, Input: &sessionloop.Input{
		Content: []sessionloop.Block{{Kind: sessionloop.BlockData, Data: json.RawMessage(`{"a":1}`)}},
	}}
...
func TestLoopViewQueueEventsAndPendingProjection(t *testing.T) {
	...
	if accepted.Queue == nil || accepted.Queue.ID != receipt.QueueID ||
		...
		len(accepted.Queue.Content) != 1 || accepted.Queue.Content[0].Text != "queued for later" {
