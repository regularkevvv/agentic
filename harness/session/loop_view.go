...
// loopInputMessage converts protocol input into one user message. Only text
// blocks are accepted in v1; any other block kind is explicitly unsupported
// rather than silently rewritten (law L9). Input.Meta is correlation-only
// and is never translated into model-visible content.
func loopInputMessage(input *sessionloop.Input) (agentic.Message, error) {
	if err := sessionloop.ValidateInput(*input); err != nil {
		return agentic.Message{}, err
	}
	parts := make([]agentic.Part, 0, len(input.Content))
...
func (v *LoopView[O]) dispatchStart(
	...
	message, err := loopInputMessage(command.Input)
	if err != nil {
		return sessionloop.Receipt{}, err
	}
...
func loopResumeRequest(command sessionloop.Command) (ResumeRequest, error) {
	...
	if command.Input != nil {
		prompt, err := loopInputMessage(command.Input)
		if err != nil {
...
func (v *LoopView[O]) newProjector(withOutput bool) *loopProjector {
