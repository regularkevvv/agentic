...
// RequestStream makes the real-host conformance environment use the same
// provider-streaming path as production StreamModel implementations. The
// provider-specific chunks become Agentic preview events immediately; only
// the completely accumulated Message is later committed as an Entry.
func (m *conformanceModel) RequestStream(ctx context.Context, request *agentic.ChatRequest) (*agentic.StreamResult, error) {
	...
	for _, part := range response.Message.Content {
		...
		case agentic.ContentText:
			for _, chunk := range conformanceStreamChunks(part.Text) {
		...
		case agentic.ContentToolUse:
			...
			for _, chunk := range conformanceStreamChunks(string(arguments)) {
				events <- agentic.StreamEvent{
...
func conformanceStreamChunks(value string) []string {
	if len(value) < 2 {
		return []string{value}
	}
	middle := len(value) / 2
...
func TestSessionLoopHostJSONLRecoveryConformance(t *testing.T) {
	...
	receipt, err := session.Dispatch(sessionLoopTestContext(t), sessionloop.Command{
		Kind: sessionloop.CommandStart, Input: &sessionloop.Input{Blocks: []sessionloop.InputBlock{{Kind: sessionloop.InputBlockText, Text: "durable hello"}}},
	})
	if err != nil {
		t.Fatal(err)
	}
	...
	for index, entry := range replayedEntries {
		...
		if entry.Origin != liveEntries[index].Origin || !reflect.DeepEqual(entry.Blocks, liveEntries[index].Blocks) {
	...
	second, err := reopened.Dispatch(sessionLoopTestContext(t), sessionloop.Command{
		Kind: sessionloop.CommandStart, Input: &sessionloop.Input{Blocks: []sessionloop.InputBlock{{Kind: sessionloop.InputBlockText, Text: "second run"}}},
	})
	if err != nil {
...
func TestSessionLoopDurableReceiptPrecedesDriverExecution(t *testing.T) {
	...
	receipt, err := session.Dispatch(sessionLoopTestContext(t), sessionloop.Command{
		Kind: sessionloop.CommandStart, Input: &sessionloop.Input{Blocks: []sessionloop.InputBlock{{Kind: sessionloop.InputBlockText, Text: "prove durability"}}},
	})
	if err != nil {
		t.Fatal(err)
	}
