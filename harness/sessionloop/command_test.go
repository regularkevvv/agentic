...
func validInput() *sessionloop.Input {
	return &sessionloop.Input{Content: []sessionloop.Block{{Kind: sessionloop.BlockText, Text: "hello"}}}
}
...
func TestCommandCloneIsDeeplyIndependent(t *testing.T) {
	...
	original := sessionloop.Command{
		...
		RunID:      "run-1",
		Input:      &sessionloop.Input{Content: []sessionloop.Block{{Kind: sessionloop.BlockText, Text: "keep"}}, Meta: map[string]string{"k": "v"}},
		Resolution: validResolution(),
	}
	clone := original.Clone()
	clone.Input.Content[0].Text = "mutated"
	clone.Input.Meta["k"] = "mutated"
	clone.Resolution.SuspensionID = "mutated"
	clone.Resolution.Decisions[0].Data[2] = 'X'
	if original.Input.Content[0].Text != "keep" || original.Input.Meta["k"] != "v" {
