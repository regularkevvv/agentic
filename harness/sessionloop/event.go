...
// Event is the sole value returned by Stream.Next. It carries common identity
// and exactly one relevant typed payload selected by Kind. Every authoritative
// event has a non-zero position when replay is advertised. Preview events may
// repeat the latest durable position and use Ordinal for arrival order.
// Dropped counts preview events lost from this subscriber's stream since its
// previous delivery.
type Event struct {
	Position   Position
	Ordinal    uint64
...
// QueuedInput is one durable caller input awaiting a future drain. Blocks keep
// their caller-to-session InputBlock shape until the queue is committed as a
// conversation Entry.
...
func (q QueuedInput) Clone() QueuedInput {
	clone := q
	clone.Blocks = cloneInputBlocks(q.Blocks)
	return clone
}
...
// Preview is an incomplete, transient, lossy progress update carried by an
// EventPreviewDelta event. It never becomes conversation truth merely by being
// observed (law L5).
type Preview struct {
	Kind       PreviewKind
	Text       string
...
// RunOutcome is the singular settled outcome carried by an EventRunSettled
// event. It is not the assistant message: assistant content is carried by
// EventEntryCommitted events. Failure is operator-facing text supplied by the
// host: its wording is not part of the contract (error strings never are — only
// Kind and the error sentinels carry identity), and hosts decide how much error
// detail they surface. Output is optional structured output present only when
// the host advertises CapabilityStructuredOutput and an application-owned
