...
func inputText(input sessionloop.Input) string {
	for _, block := range input.Content {
		if block.Kind == sessionloop.BlockText {
			return block.Text
		}
	}
	return ""
...
func (rc *RunContext) EmitAssistant(text string) {
	...
	rc.commit(sessionloop.RoleAssistant, sessionloop.OriginAssistant, delta, sessionloop.Block{
		Kind: sessionloop.BlockText,
		Text: text,
	})
}
...
func (rc *RunContext) EmitToolCall(callID, name, dataJSON string) {
	rc.commit(sessionloop.RoleAssistant, sessionloop.OriginAssistant, sessionloop.Usage{ToolCalls: 1}, sessionloop.Block{
		Kind:     sessionloop.BlockToolCall,
		ToolCall: &sessionloop.ToolCall{CallID: callID, Name: name, Data: mustRaw("EmitToolCall", dataJSON)},
	})
}
...
func (rc *RunContext) EmitToolResult(callID, name, text string, isError bool) {
	rc.commit(sessionloop.RoleTool, sessionloop.OriginTool, sessionloop.Usage{}, sessionloop.Block{
		Kind:       sessionloop.BlockToolResult,
		Text:       text,
		ToolResult: &sessionloop.ToolResult{CallID: callID, Name: name, IsError: isError},
	})
}

func (rc *RunContext) commit(role sessionloop.Role, origin sessionloop.EntryOrigin, delta sessionloop.Usage, blocks ...sessionloop.Block) {
	st := rc.state
	st.mu.Lock()
	defer st.mu.Unlock()
	if st.run != rc.run {
