...
func (st *sessionState) acceptStartLocked(command sessionloop.Command) (sessionloop.Receipt, error) {
	...
	for _, queued := range st.pending {
		...
		st.commitEntryLocked(sessionloop.RoleUser, sessionloop.OriginNextTurn, runID, queued.CommandID, queued.Content)
	}
	st.pending = nil
	st.commitEntryLocked(sessionloop.RoleUser, sessionloop.OriginStart, runID, command.ID, command.Input.Content)
...
func (st *sessionState) acceptDeliveryLocked(command sessionloop.Command) (sessionloop.Receipt, error) {
	...
	position := st.commitEntryLocked(sessionloop.RoleUser, origin, run.id, command.ID, command.Input.Content)
...
func (st *sessionState) acceptResolveLocked(command sessionloop.Command) (sessionloop.Receipt, error) {
	...
	st.appendLocked(sessionloop.Event{Kind: sessionloop.EventCommandAccepted, RunID: run.id, CommandID: command.ID})
	if command.Input != nil {
		st.commitEntryLocked(sessionloop.RoleUser, sessionloop.OriginFollowUp, run.id, command.ID, command.Input.Content)
	}
	run.suspension = nil
	st.state = sessionloop.StateRunning
...
func (st *sessionState) commitEntryLocked(role sessionloop.Role, origin sessionloop.EntryOrigin, runID sessionloop.RunID, commandID sessionloop.CommandID, content []sessionloop.Block) sessionloop.Position {
