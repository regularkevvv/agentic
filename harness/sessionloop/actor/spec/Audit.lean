...
elab "#audit_session_contract" : command => do
  let required := #[
    ...
    ``SessionContract.RecoveryFrontier.reachable_safe,
    ``SessionContract.RecoveryFrontier.reachable_identity,
    ``SessionContract.RecoveryFrontier.acknowledged_steering_not_lost,
    ``SessionContract.RecoveryFrontier.closed_steering_was_applied,
    ``SessionContract.RecoveryFrontier.recovery_keeps_durable_frontier,
    ``SessionContract.RecoveryFrontier.recovery_continuation_is_legal,
    ``SessionContract.RecoveryFrontier.every_prefix_recoverable,
    ``SessionContract.RecoveryFrontier.old_dispatch_rejects_reachable_candidate,
    ``SessionContract.Cleanup.reachable_safe,
    ``SessionContract.Cleanup.abandon_preserves_durable,
    ``SessionContract.Cleanup.late_request_cancel_preserves_durable,
    ``SessionContract.Cleanup.late_finalizers_cannot_write,
    ``SessionContract.Cleanup.worker_loss_can_recover,
    ``SessionContract.Cleanup.old_cleanup_breaks_contract,
