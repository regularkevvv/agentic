// Package memory is the process-local receive flavor of the actor contract.
...
func (s *Store) Acquire(ctx context.Context, id actor.ActorID, owner string, ttl time.Duration) (actor.Lease, error) {
	...
	if id == "" || owner == "" || ttl <= 0 {
		return actor.Lease{}, errors.New("memory actor: identity and positive TTL required")
	}
	s.mu.Lock()
	defer s.mu.Unlock()
	state := s.sessions[id]
...
func (s *Store) Renew(ctx context.Context, lease actor.Lease, ttl time.Duration) (actor.Lease, error) {
	...
	if ttl <= 0 {
		return actor.Lease{}, errors.New("memory actor: positive TTL required")
	}
	s.mu.Lock()
	defer s.mu.Unlock()
