...
func casePreview(t *testing.T, env Env, _ sessionloop.Capabilities) {
	...
	for index, event := range seen {
		if event.Nature == sessionloop.EventPreview {
			...
			if event.Preview == nil {
				t.Fatal("preview.delta event has no Preview payload")
			}
		}
		if event.Kind == sessionloop.EventEntryCommitted && event.Entry != nil &&
			event.Entry.Role == sessionloop.RoleAssistant && assistantEntryIndex < 0 {
	...
	if assistantEntryIndex < 0 {
		t.Fatal("the preview scenario emitted no complete assistant Entry")
	}
	if firstPreviewIndex >= assistantEntryIndex || assistantEntryIndex >= settledIndex {
		t.Fatalf("streaming boundary order: preview=%d assistant entry=%d settlement=%d; want Preview before complete Entry before RunOutcome",
			firstPreviewIndex, assistantEntryIndex, settledIndex)
	}
...
func caseSuspensionSurvivesCloseAndReopen(t *testing.T, env Env, _ sessionloop.Capabilities) {
	...
	beforeClose := snapshotOf(t, session)
	if beforeClose.State != sessionloop.StateSuspended || beforeClose.Suspension == nil ||
		beforeClose.Suspension.ID != suspended.Suspension.ID {
		t.Fatalf("run.suspended was observable before the session finalized its suspension: %#v", beforeClose)
	}
...
func caseDetailedTools(t *testing.T, env Env, _ sessionloop.Capabilities) {
	...
	var call *sessionloop.EntryToolCall
	var result *sessionloop.EntryToolResult
	for _, entry := range committedEntries(seen, receipt.RunID) {
		for _, block := range entry.Blocks {
			switch block.Kind {
			case sessionloop.EntryBlockToolCall:
				call = block.ToolCall
			case sessionloop.EntryBlockToolResult:
