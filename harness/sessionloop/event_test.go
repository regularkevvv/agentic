...
func fullEvent() sessionloop.Event {
	return sessionloop.Event{
		...
		Entry: &sessionloop.Entry{
			...
			Blocks: []sessionloop.EntryBlock{{Kind: sessionloop.EntryBlockText, Text: "keep"}},
		},
		Queue: &sessionloop.QueuedInput{
			...
			Blocks:    []sessionloop.InputBlock{{Kind: sessionloop.InputBlockText, Text: "queued"}},
		},
...
func TestEventCloneDeepCopiesEveryTypedPayload(t *testing.T) {
	...
	clone.Entry.Blocks[0].Text = "mutated"
	clone.Queue.Blocks[0].Text = "mutated"
...
func TestPayloadClonesAreDeeplyIndependent(t *testing.T) {
	t.Parallel()
	queued := sessionloop.QueuedInput{ID: "queue-1", Blocks: []sessionloop.InputBlock{{Kind: sessionloop.InputBlockText, Text: "keep"}}}
	queuedClone := queued.Clone()
	queuedClone.Blocks[0].Text = "mutated"
	if queued.Blocks[0].Text != "keep" {
...
func TestSnapshotCloneIsDeeplyIndependent(t *testing.T) {
	...
	original := sessionloop.Snapshot{
		...
		ActiveRunID:  "run-1",
		Entries:      []sessionloop.Entry{{ID: "entry-1", Blocks: []sessionloop.EntryBlock{{Kind: sessionloop.EntryBlockText, Text: "keep"}}}},
		Pending:      []sessionloop.QueuedInput{{ID: "queue-1", Blocks: []sessionloop.InputBlock{{Kind: sessionloop.InputBlockText, Text: "keep"}}}},
		Suspension:   suspension,
		Usage:        sessionloop.Usage{TotalTokens: 7},
		Capabilities: sessionloop.NewCapabilities(sessionloop.CapabilitySteer),
	}
	clone := original.Clone()
	clone.Entries[0].Blocks[0].Text = "mutated"
	clone.Pending[0].Blocks[0].Text = "mutated"
	clone.Suspension.Decisions[0].Name = "mutated"
	clone.Capabilities[0] = "mutated"
	if original.Entries[0].Blocks[0].Text != "keep" ||
		original.Pending[0].Blocks[0].Text != "keep" ||
