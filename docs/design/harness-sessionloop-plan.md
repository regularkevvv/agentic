...
Agentic Harness, the Agentic TUI bridge, and external consumers such as Chacla.
It does not execute a model, import Agentic, know about Chacla, or become a
...
- a clean external consumer can use `sessionloop` without pulling root Agentic,
  Harness, TUI, provider SDKs, or terminal dependencies. Chacla adoption is the
  downstream acceptance proof after the Agentic modules are released.
...
package would still place Agentic in every Chacla consumer's module graph.
...
| **Entry** | One authoritative committed conversation item, including user, assistant, and tool content. |
| **Preview** | A transient, lossy progress update that never becomes transcript truth merely by being observed. |
...
1. Define one asynchronous interaction contract that does not import Agentic,
   Harness, TUI, Chacla, a provider SDK, or a transport library.
...
- Making Chacla a dependency of Agentic.
- Replacing Chacla's principal attachment contract. Session facade mode and
  attachment mode remain separate integration forms.
...
```text
                  github.com/.../harness/sessionloop
                       no project dependencies
                         ▲       ▲       ▲
                         │       │       │
        Agentic Harness ─┘       │       └─ Chacla adapter satellite
                                 │
                         Agentic TUI bridge
```
...
Standard block kinds are text, data, tool call, and tool result. Tool calls and
results have stable call IDs, names, error state, and optional JSON data. An
adapter validates that `Data` contains one complete JSON value before exposing
it. `json.RawMessage` is copied at every API boundary.
...
Metadata is for correlation, not instruction smuggling. Implementations must not
...
- `agentic.Message` -> copy-owned `sessionloop.Entry` blocks;
...
## 10. Chacla consumption boundary

Chacla is a downstream consumer and acceptance proof, not a dependency or a
package in this repository.

The intended downstream import belongs in Chacla's dependency-bearing harness
adapter module, never its zero-dependency root:
...
```text
Chacla principal binding                 sessionloop host
fresh trusted instructions + tools       commands + events + snapshots
                 │                                  │
                 └──────── concrete principal ──────┘
                                      │
                              Chacla facade session
```

The shared protocol does not contain memory concepts. Chacla remains
responsible for:
...
The Agentic implementation is downstream-ready only when Chacla can:
...
This proof should live in Chacla's satellite tests after the Agentic releases.
Agentic additionally carries a Chacla-shaped, dependency-free conformance
consumer so the protocol cannot regress between downstream releases.
...
4. update Chacla's harness adapter module and run its facade acceptance suite.
...
Only after the released protocol, TUI bridge, and Chacla consumer are stable,
...
**Risk:** types ship but the real Harness and Chacla continue using unrelated
wrappers.

**Mitigation:** S4 real Harness conformance and S6 Chacla acceptance are part of
the delivery definition, not optional examples.
...
tag, then TUI and Chacla. Release-view tests run with `GOWORK=off`.
...
| Does Chacla core import the module? | No. Its dependency-bearing harness adapter does. |
| Does this replace Chacla attachment mode? | No. It enables facade mode; attachment remains a separate contract. |
...
- [ ] A fresh sessionloop-only consumer has no Agentic/Harness/TUI/provider
      dependencies.
- [ ] Chacla's harness satellite consumes the published protocol, runs a prompt,
      steers the active run, observes one authoritative settlement, returns to
      idle, starts a second run, and records without duplication.
- [ ] Documentation clearly distinguishes session facade mode, principal
      attachment mode, and the existing Agentic blocking API.

Until the final Chacla acceptance item is green, the Agentic implementation may
be released and useful, but the cross-repository objective described by this
