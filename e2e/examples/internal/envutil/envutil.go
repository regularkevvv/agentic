...
// every example whether it is run as `go run ./e2e/examples/basic` from the
// root or as `go run .` from inside the example's own directory. Looking only
// in the working directory made the second form fail with a confusing
