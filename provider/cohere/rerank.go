...
// Reranker implements retrieval.Reranker using the Cohere /v2/rerank API.
...
// returns an id, which retrieval.RerankResponse has nowhere to put.
...
func (r *Reranker) Rerank(ctx context.Context, req *retrieval.RerankRequest) (*retrieval.RerankResponse, error) {
	...
	// TopN larger than the document count is clamped, as retrieval.RerankRequest
	...
	results := make([]retrieval.RerankResult, 0, len(resp.Results))
	...
	// documents that it returns results ranked, but retrieval.RerankResponse promises
	...
	return &retrieval.RerankResponse{
		...
		Usage: retrieval.RerankUsage{SearchUnits: resp.Meta.BilledUnits.SearchUnits},
	}, nil
}
