...
func TestSnapshotProjectionAndDecoration(t *testing.T) {
	...
	inner := &fakeSession{id: "sess-2", snapshots: []snapshotStep{{snapshot: sl.Snapshot{
		...
		Entries: []sl.Entry{
			{Role: sl.RoleUser, Origin: sl.OriginStart, Blocks: []sl.EntryBlock{
				{Kind: sl.EntryBlockText, Text: "hi "}, {Kind: sl.EntryBlockText, Text: "there"},
				{Kind: sl.EntryBlockData, MediaType: "application/json", Data: []byte(`{"x":1}`)},
			}},
			{Role: sl.RoleAssistant, Origin: sl.OriginAssistant, Blocks: []sl.EntryBlock{
				{Kind: sl.EntryBlockToolCall, ToolCall: &sl.EntryToolCall{CallID: "c1", Name: "lookup", Data: []byte(`{"secret":1}`)}},
				{Kind: sl.EntryBlockToolCall},
			}},
			{Role: sl.RoleTool, Origin: sl.OriginTool, Blocks: []sl.EntryBlock{
				{Kind: sl.EntryBlockToolResult, Text: "raw result", ToolResult: &sl.EntryToolResult{CallID: "c1", Name: "lookup", IsError: true}},
				{Kind: sl.EntryBlockToolResult},
			}},
		},
		Pending: []sl.QueuedInput{
			{ID: "q1", Kind: sl.CommandNextTurn, Blocks: []sl.InputBlock{{Kind: sl.InputBlockText, Text: "queued"}}},
			{ID: "q2", Kind: sl.CommandSteer, Blocks: []sl.InputBlock{{Kind: sl.InputBlockData, Data: []byte(`1`)}}},
		},
...
func TestSubmitWaitsForTheRightRunBoundary(t *testing.T) {
	...
	if len(commands) != 1 || commands[0].Kind != sl.CommandStart || commands[0].Input.Blocks[0].Text != "go" {
...
func TestResolveValidationAndDispatch(t *testing.T) {
	...
	if commands[0].Input == nil || commands[0].Input.Blocks[0].Text != "carry on" {
