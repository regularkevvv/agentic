...
func resultTool(value sl.EntryToolResult) uit.Tool {
	state := uit.ToolDone
	if value.IsError {
		state = uit.ToolError
	}
...
func queuedInput(value sl.QueuedInput) uit.QueuedInput {
	return uit.QueuedInput{ID: string(value.ID), Kind: uit.QueueKind(value.Kind), Text: firstInputText(value.Blocks)}
}

func firstInputText(blocks []sl.InputBlock) string {
	for _, block := range blocks {
		if block.Kind == sl.InputBlockText {
			return block.Text
		}
	}
	return ""
...
func entryEvent(base uit.Event, value *sl.Entry, presenter uit.ToolPresenter) []uit.Event {
	...
	case sl.OriginTool:
		...
		for _, block := range value.Blocks {
			if block.Kind == sl.EntryBlockToolResult && block.ToolResult != nil {
